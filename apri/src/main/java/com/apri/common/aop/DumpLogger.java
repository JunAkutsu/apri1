package com.apri.common.aop;

import com.apri.common.context.*;
...
public class DumpLogger {
    /** 設定対象 */
    public static final String TARGET_ALL = "execution(* com.apri..*.*(..))"
            + " && (bean(*Controller) || bean(*Logic) || bean(*Dao))";

    /** ロガー */
    protected static final Logger LOGGER = LoggerFactory.getLogger("dump_logger");
    ...
    public static Object dump(ProceedingJoinPoint joinPoint, Context context) throws Throwable {
        ...
        try {
            ...
            LOGGER.trace("with args - " + ToStringBuilder.reflectionToString(joinPoint.getArgs(),ToStringStyle.SHORT_PREFIX_STYLE));
            Object retValue = joinPoint.proceed(joinPoint.getArgs());
            LOGGER.debug("END[" + getSessionId(context) + "] - " + toCall(joinPoint));
            LOGGER.trace("with return - " + ToStringBuilder.reflectionToString(retValue, ToStringStyle.SHORT_PREFIX_STYLE));
            return retValue;
        } catch (Throwable th) {
