...
public class SimpleContainerLogic {
	...
	@RequestMapping(value="/search_team_list",consumes=MediaType.APPLICATION_JSON_UTF8_VALUE,method=RequestMethod.POST)
	@ResponseBody
	public String ajax_search_team_list(){
		// 初期化
		...
		// 予定リストの取得
		TeamDomain input = new TeamDomain();
		
		List<TeamDomain> team_list = teamService.getTeamList(input);
		
		// ログ出力
		Logger logger = LoggerFactory.getLogger(RegistContainerLogic.class);
		
		return gson.toJson(team_list);
	}
	...
	public String ajax_search_calender_list(@RequestBody YoteiBean yoteiBean){
		...
		List<YoteiDomain> yotei_list = teamService.getTeamYoteiList(input);
...
//		v_result.put("id", input.getId());
	...
	public String ajax_modify(@RequestBody YoteiBean yoteiBean){
		...
		// 予定の取得
		YoteiDomain yoteiInfo = teamService.getTeamYotei(yoteiBean.getId());
		
		// ログ出力
		Logger logger = LoggerFactory.getLogger(RegistContainerLogic.class);
		
		v_result.put("id", yoteiInfo.getId());
		v_result.put("resourceId", yoteiInfo.getResourceId());
	...
	public String ajax_delete(@RequestBody YoteiBean yoteiBean){
		...
		// 予定の削除
		teamService.delete(yoteiBean.getId());
